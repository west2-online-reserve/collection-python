...
@router.get("/follower/list")
async def get_user_follower(user_id: int = Query(..., ge=1),
    ...
    # 计算总记录数
    async def find2():
        res = db.query(func.count(Follower.follower_id)).filter(Follower.following_id == user_id).scalar()
        return res
    ...
    async def find3():
        res = follower_users_query.all()
        return res

    follower_users = await find3()
    res_item = []
    for item in follower_users:
        res_item.append({
            "id": item.id,
